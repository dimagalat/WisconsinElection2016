...
setwd("https://github.com/Sean-Case/WisconsinElection2016") # Change this to the location of your downloaded data files
...
```
* The Wisconsin election results did not deviate significantly from historical trends.
* The increase in the 'other' vote, despite matching national trends, seemed to be unusual when considering results on the county level, especially when looking at counties with voting machines installed.
* However, municipality / ward level voting statistics are needed to disentangle the (potential) effects of the presence of voting machines from that of county population size.
...
Concern has surfaced by those close to the events of election night, specifically that the USA was under attack from hackers during November 8. Some feared that hacking may have had some influence on the election, especially in areas where voting machines are in use. Since the result in Wisconsin was so close, and the result so unexpected (at least judging by the polls close to election day), I decided to use my R skills to try to look deeper into the Wisconsin results. All the data and code used for this analysis is provided in the GitHub repository https://github.com/Sean-Case/Sean-Case.github.io/tree/master/wisconsinvotes

Anyway, back to the Wisconsin election. In order to investigate the possibility of manipulation in the Wisconsin 2016 Presidential elections, we need to ask a few questions.
...
candidate.2016.summary.vis.defs = ggplot(candidate.2016.summary,
  ...
  scale_y_continuous(
    name =
      "Bars: Votes for the candidates (%).<br>Lines: polling of decided voters (%)."
  ) +
  scale_x_discrete(
...
## Was Wisconsin 2016 a strange election compared to historical trends?
...
This above figure shows us that 1. there were a lot less counties won by Clinton than Trump, and 2. the counties won by Clinton did not necessarily have a lower turnout. We can confirm this with a scatter graph. The following shows that the Clinton vote did not drop with turnout percentage in each county.
...
The following figure shows that although there was some variation in the vote for 'other' candidates by county, it did not seem to be correlated with the change in voter turnout from 2012.

```{r other votes vs turnout, echo = F, fig.cap = "*Votes for 'other' candidates in 2016 by county and by change in turnout from 2012 (%).*", fig.height=6, fig.width=10}
...
county.perc.winner.other = ggplot(
  ...
  geom_bar(stat = "identity", alpha = 0.6) +
  scale_y_continuous(name =
                       paste("Percentage vote for 'Other' in Wisconsin counties.<br>Grey bar behind indicates change in turnout from 2012 (%)")) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 7)) +
  scale_x_discrete(
...
```
The following figure shows that in many counties, the vote swung significantly from Democrat to Republican from the 2012 election. However, there is no single county that stands out as an outlier in terms of an unusual switch from Democrat to Republican between 2012 and 2016.
...
county.2012v2016.perc.diff.2016ord = ggplot(county.2016.vs.2012,
  ...
  scale_y_continuous(
    name = "Vote difference in 2012 and 2016 by county (%),<br>Negative = Democrat, positive = Republican ") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 7)) +
  scale_x_discrete(
...
```
In this section we have shown that although there was a significant swing from Democrat to Republican in many counties, no single county stands out as unusual compared to the others in terms of magnitude of votes for one party or another, vote swing from one party to another, change in turnout. However, in such a close election race, only small amounts of manipulation would be necessary to swing a few thousand votes here and there to change the result.

## Voting machines
One interesting dataset provided by the Wisconsin Election Commission is the presence of voting machines on a ward-by-ward basis. I would assume that any manipulation of the election results would be more likely to occur in wards / counties that have a higher proportion of voting machines. I have incorporated this information into the data.
...
In order to determine the relative influence of many interacting factors on the vote in each county, it is important to consider them side by side. One of the ways to quickly determine which factors are linked is by using a correlation matrix.

I have summarised the interaction of voting machines with several other variables we have already considered in a correlation matrix below. Numbers closer to 1 or -1 indicate a stronger positive or negative correlation respectively. 
...
Now we can see that the change in vote for other between 2012 and 2016 is highly correlated with the log of the turnout. So, we are back to step 1. Since the results are not consistent when using the original turnout numbers or the logged turnout numbers, we cannot conclude that the other vote change was linked to voting machines, and not just overall turnout. This outcome is illustrated below.

```{r correlation turnout with other, echo=F, fig.cap="*The correlation between the change in vote for other candidates between 2012 and 2016 with the voter turnout by county, on left non-transormed, and on right transformed by the natural log*"}
...
# {
  county.2012v2016.change.oth.turnout.point = ggplot(county.summary.2016.final.2000ppl,
    ...
    scale_y_continuous(
      name = "Change in vote<br>for other 2012 - 2016 (%)"
    ) +
    scale_x_continuous(
...
What we really need is municipality or ward-level data in order to remove the effect of greatly-varying local population numbers. Until this data is available, we can only show general descriptive graphs of the Wisconsin vote, and I will highlight a few interesting graphs of the 'other' vote that may be worth looking into in greater depth later, if more data becomes available.

## Analysing county results 2000 - 2016 vs voting machine presence in 2016

The following figures show the change in votes for 'Other' candidates with time according to the use of machines or the by the previous voting record of the county (Democrat or Repulican). Although the vote percentage for 'other' candidates is generally higher in 2016 than in earlier elections, and is even higher for counties that use voting machines, the difference is similar whether the county has mostly voted Democrat or Republican since 2000.

If the election was intentionally manipulated in some way, I would have expected to see a difference in the voting results according to both the proportion of voting machines in the county AND by the general winning party in the county. But that's just an assumption on my part :).

```{r correlationgraph machines other vote, echo=F, fig.cap="*Other vote by use of voting machines in county (%)*", fig.height=10, fig.width=6, message=FALSE, warning=FALSE}
...
```{r correlationgraph machines other change, echo=F, fig.cap="*Change in other vote (%) by use of voting machines in county (%), fig.height=10, fig.width=6, message=FALSE, warning=FALSE, and the counties that have chosen mostly Democrat or Republican since 2000. Points are individual county results, lines are a mean of the counties in each category.*", warning=F}
...
```{r correlationgraph machines other numbers, echo=F, fig.cap="*Change in other vote (number of people) by use of voting machines in county (%), fig.height=10, fig.width=6, message=FALSE, warning=FALSE, and the counties that have chosen mostly Democrat or Republican since 2000. Points are individual county results, lines are a mean of the counties in each category. The error bars are particularly large here because of Milwaukee and Dane counties.*", warning=F}
